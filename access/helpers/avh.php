...
class avh_Core {
	...
	public static function vform()
	{
        ...
        $html = '';
        $genders = ORM::factory('victim_gender')->select_list('id','gender');
        array_unshift($genders,'');
        $status = ORM::factory('victim_status')->select_list('id','status');
        array_unshift($status,'');
        $occupation = ORM::factory('victim_occupation')->select_list('id','occupation');
        ...
        foreach ($acs as $_ac){
            ...
            foreach ($_ac['h'] as $_ach){
                ...
                foreach ($_ach['h'] as $_idn => $_nn){
                    $_forn = "nieto_$_idn";
                    $dwn .= "<li data=\"ida:".$_idn."\">".ucfirst(strtolower($_nn))."</li>";
                }
                $dwn .= '</ul></li>';
            }
            $dwn .= '</ul>';
