<?php
/**
 * Performs install/uninstall methods for the actionable plugin
 *
 * PHP version 5
 * LICENSE: This source file is subject to LGPL license 
 * that is available through the world-wide-web at the following URI:
 * http://www.gnu.org/copyleft/lesser.html
 * @author	   Ushahidi Team <team@ushahidi.com> 
 * @package    Ushahidi - http://source.ushahididev.com
 * @module	   Actionable Installer
 * @copyright  Ushahidi - http://www.ushahidi.com
 * @license    http://www.gnu.org/copyleft/lesser.html GNU Lesser General Public License (LGPL) 
 */

class State_Install {

	/**
	 * Constructor to load the shared database library
	 */
	public function __construct()
	{
		$this->db = Database::instance();
	}

	/**
	 * Creates the required database tables for the actionable plugin
	 */
	public function run_install()
	{
        $sql = "
        CREATE TABLE IF NOT EXISTS ".Kohana::config('database.default.table_prefix')." state (
          `id` int(11) NOT NULL AUTO_INCREMENT,
          `state` varchar(100) DEFAULT NULL,
          `divipola` varchar(5) DEFAULT NULL,
          `country_id` int(11) NOT NULL,
          PRIMARY KEY (`id`),
          KEY `fk_state_country1` (`country_id`)
        ) ENGINE=MyISAM  DEFAULT CHARSET=utf8;
        ";

        $this->db->query($sql);

        $sql = "ALTER TABLE location ADD COLUMN state_id INT(4) NOT NULL";
        $this->db->query($sql);

	}

	/**
	 * Deletes the database tables for the actionable module
	 */
	public function uninstall()
	{
    }
}
